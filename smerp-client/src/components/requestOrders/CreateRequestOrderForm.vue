<template>
  <div class="card shadow-sm">
    <div class="card-body">
      <h5 class="card-title mb-4">새 발주 등록</h5>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <tr v-for="(item, index) in form.items" :key="index">
              ...
              <td><input type="number" v-model="item.inboundUnitPrice" class="form-control form-control-sm" step="0.01" min="0" required></td>
...
<script setup>
import { ref } from 'vue';
import { createRequestOrder } from "@/api/requestOrder";
...
function handleClientSelect(client) {
  form.value.companyName = client.companyName; // Assuming client object has a 'companyName' property
}
...
async function submitForm() {
  try {
    ...
    const response = await createRequestOrder(orderData);
    console.log('Request order created successfully:', response.data);
    emit('save');
  } catch (error) {
    console.error('Error creating request order:', error);
    alert('발주 생성에 실패했습니다: ' + (error.response?.data?.message || error.message));
  }
}
